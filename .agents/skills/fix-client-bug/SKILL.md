---
name: fix-client-bug
description: Fix bugs or add features in the rtd-typescript client layer (gRPC, JSON-RPC, GraphQL). Ensures all three transport implementations stay in sync.
user-invocable: true
...
Use this workflow when making changes to Core API methods in `packages/rtd/src/`. The key architectural constraint is that the three transport implementations (gRPC, JSON-RPC, GraphQL) must always produce identical results for the same Core API call.

## Phase 1: Read Before Writing

1. Identify the affected Core API method(s) in `packages/rtd/src/client/core.ts`.
2. Read the shared return types in `packages/rtd/src/client/types.ts`.
3. Read **all three** transport implementations of the method:
   - `packages/rtd/src/grpc/core.ts`
   - `packages/rtd/src/jsonRpc/core.ts`
   - `packages/rtd/src/graphql/core.ts`
...
- Use explicit return type annotations in the mapping functions (e.g., `(item): RtdClientTypes.SomeType => { ... }`) to catch type mismatches at the implementation site.
...
- Regenerate types: `pnpm --filter rtd-typescript codegen:graphql`
...
1. Add or update tests in `packages/rtd/test/e2e/clients/core/`.
...
1. Build: `pnpm turbo build --filter=rtd-typescript`
2. Lint: `pnpm --filter rtd-typescript oxlint:check`
3. Unit tests: `pnpm --filter rtd-typescript test`
