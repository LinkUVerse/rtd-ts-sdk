...
```mdx
---
title: Rtd Programmable Transaction Basics
...
SDK developers building Rtd applications in TypeScript. Assume familiarity with TypeScript and npm/pnpm but not with Rtd-specific concepts.
...
- Use public API paths: `rtd-typescript/transactions`, not `../src/transactions`
...
```mdx
The `ClientWithCoreApi` type from `rtd-typescript/client` provides...
```
...
```mdx
<AutoTypeTable path="rtd-typescript" name="TransactionOptions" />
```
...
---
title: Payment Kit
description: Accept payments in any coin type on Rtd
---
