...
import { RtdSignPersonalMessage } from 'rtd-wallet-standard';
import type {
...
} & Omit<RtdSignPersonalMessageInput, 'account' | 'chain'>;
...
export function signPersonalMessageCreator<TNetworks extends Networks>({
	...
	return async function signPersonalMessage({
		...
		const signPersonalMessageFeature = getAccountFeature({
			account,
			chain,
			featureName: RtdSignPersonalMessage,
		}) as RtdSignPersonalMessageFeature[typeof RtdSignPersonalMessage];
