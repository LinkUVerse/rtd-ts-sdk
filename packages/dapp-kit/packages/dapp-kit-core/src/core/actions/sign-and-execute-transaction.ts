...
} & Omit<RtdSignAndExecuteTransactionInput, 'account' | 'chain' | 'transaction'>;
...
export function signAndExecuteTransactionCreator<TNetworks extends Networks>(
	...
	return async function signAndExecuteTransaction({
		...
		const signAndExecuteTransactionFeature = tryGetAccountFeature({
			account,
			chain,
			featureName: RtdSignAndExecuteTransaction,
		}) as RtdSignAndExecuteTransactionFeature[typeof RtdSignAndExecuteTransaction];
		...
		const signAndExecuteTransactionBlockFeature = tryGetAccountFeature({
			account,
			chain,
			featureName: RtdSignAndExecuteTransactionBlock,
		}) as RtdSignAndExecuteTransactionBlockFeature[typeof RtdSignAndExecuteTransactionBlock];
