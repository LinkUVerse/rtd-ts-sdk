...
import type { StandardConnectInput, RtdWalletFeatures } from 'rtd-wallet-standard';
import type { StandardConnectFeature } from 'rtd-wallet-standard';
import { StandardConnect } from 'rtd-wallet-standard';
	...
	return async function connectWallet({
		...
		return await task(async () => {
			...
			try {
				...
				const { accounts: rtdAccounts, supportedIntents } = await internalConnectWallet(
					wallet,
					supportedNetworks,
					standardConnectArgs,
				);

				if (!isAlreadyConnected && rtdAccounts.length === 0) {
				...
				const selectedAccount = account ?? rtdAccounts[0];
...
export async function getSupportedIntentsFromFeature(wallet: UiWallet) {
	if (!wallet.features.includes('rtd:getCapabilities')) {
		return [];
	}
	...
	const getCapabilitiesFeature = getWalletFeature(
		wallet,
		'rtd:getCapabilities',
	) as RtdWalletFeatures['rtd:getCapabilities'];
