...
import type { Wallet } from 'rtd-wallet-standard';
import { toBase64 } from 'rtd-utils';
import type { ClientWithCoreApi } from 'rtd-typescript/client';
...
export class UnsafeBurnerWallet implements Wallet {
	...
	constructor({ clients }: { clients: ClientWithCoreApi[] }) {
		...
		this.#account = new ReadonlyWalletAccount({
			address: this.#keypair.getPublicKey().toRtdAddress(),
			publicKey: this.#keypair.getPublicKey().toRtdBytes(),
			chains: this.chains,
			features: [RtdSignTransaction, RtdSignAndExecuteTransaction, RtdSignPersonalMessage],
		});
	...
	#signTransaction: RtdSignTransactionMethod = async ({ transaction, signal, chain }) => {
	...
	#signAndExecuteTransaction: RtdSignAndExecuteTransactionMethod = async ({
		transaction,
		signal,
		chain,
	}) => {
