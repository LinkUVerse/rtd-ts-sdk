...
The Rtd dApp Kit is a set of React components, hooks, and utilities that make it easy to build a
dApp for the Rtd ecosystem. It provides hooks and components for querying data from the Rtd
blockchain, and connecting to Rtd wallets.

See https://sdk.linkuverse.com/typescript for full documentation

### Core Features

- **Query Hooks:** dApp Kit provides a set of hooks for making rpc calls to the Rtd blockchain,
...
- **Supports all Rtd wallets:** No need to manually define wallets you support. All Rtd wallets are
...
To use the Rtd dApp Kit in your project, run the following command in your project root:

```sh npm2yarn
npm i --save rtd-dapp-kit @linku/rtd @tanstack/react-query
```
...
```tsx
import { createNetworkConfig, RtdClientProvider, WalletProvider } from 'rtd-dapp-kit';
import { getFullnodeUrl, type RtdClientOptions } from '@linku/rtd/client';
...
documentation pages for the components and [themes](https://sdk.linkuverse.com/dapp-kit/themes).
...
The dApp Kit provides a set of hooks for making RPC calls to the Rtd blockchain. The hooks are thin
...
function MyComponent() {
	const { data, isPending, error, refetch } = useRtdClientQuery('getOwnedObjects', {
		owner: '0x123',
	});
