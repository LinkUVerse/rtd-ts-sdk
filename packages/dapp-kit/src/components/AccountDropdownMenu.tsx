...
import { useResolveRtdNSName } from '../hooks/useResolveRtdNSNames.js';
...
export function AccountDropdownMenu({ currentAccount }: AccountDropdownMenuProps) {
	...
	const { data: domain } = useResolveRtdNSName(
...
export function AccountDropdownMenuItem({
	...
	const { data: domain } = useResolveRtdNSName(account.label ? null : account.address);
