...
import type { RtdReportTransactionEffectsInput } from 'rtd-wallet-standard';
...
type UseReportTransactionEffectsArgs = Omit<
	PartialBy<RtdReportTransactionEffectsInput, 'account' | 'chain'>,
	'effects'
> & {
	...
	return useMutation({
		...
		mutationFn: async ({ effects, chain = currentWallet?.chains[0], account = currentAccount }) => {
			...
			const reportTransactionEffectsFeature =
				currentWallet.features['rtd:reportTransactionEffects'];
