...
import { useRtdClient } from '../useRtdClient.js';
...
export function useUnsafeBurnerWallet(enabled: boolean) {
	...
	useEffect(() => {
		...
		const unregister = registerUnsafeBurnerWallet(rtdClient);
		return unregister;
	}, [enabled, rtdClient]);
}

function registerUnsafeBurnerWallet(rtdClient: RtdClient) {
	...
	class UnsafeBurnerWallet implements Wallet {
		...
		get features(): StandardConnectFeature & StandardEventsFeature & RtdFeatures {
			return {
				'standard:connect': {
					version: '1.0.0',
		...
		#signPersonalMessage: RtdSignPersonalMessageMethod = async (messageInput) => {
			const { bytes, signature } = await keypair.signPersonalMessage(messageInput.message);
			return { bytes, signature };
		};

		#signTransactionBlock: RtdSignTransactionBlockMethod = async (transactionInput) => {
		...
		#signTransaction: RtdSignTransactionMethod = async (transactionInput) => {
		...
		#signAndExecuteTransactionBlock: RtdSignAndExecuteTransactionBlockMethod = async (
			transactionInput,
		) => {
			const { bytes, signature } = await transactionInput.transactionBlock.sign({
				client: rtdClient,
			...
			return rtdClient.executeTransactionBlock({
				signature,
		...
		#signAndExecuteTransaction: RtdSignAndExecuteTransactionMethod = async (transactionInput) => {
			const { bytes, signature } = await Transaction.from(
				await transactionInput.transaction.toJSON(),
			).sign({
				client: rtdClient,
			...
			const { rawEffects, digest } = await rtdClient.executeTransactionBlock({
				signature,
				transactionBlock: bytes,
				options: {
					showRawEffects: true,
