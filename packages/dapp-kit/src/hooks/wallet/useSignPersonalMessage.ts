...
type UseSignPersonalMessageArgs = PartialBy<RtdSignPersonalMessageInput, 'account' | 'chain'>;

type UseSignPersonalMessageResult = RtdSignPersonalMessageOutput;
...
> {
	...
	return useMutation({
		...
		mutationFn: async (signPersonalMessageArgs) => {
			...
			const signPersonalMessageFeature = currentWallet.features['rtd:signPersonalMessage'];
			if (signPersonalMessageFeature) {
				return await signPersonalMessageFeature.signPersonalMessage({
					...signPersonalMessageArgs,
					account: signerAccount,
					chain: signPersonalMessageArgs.chain ?? `rtd:${network}`,
				});
			}

			// TODO: Remove this once we officially discontinue rtd:signMessage in the wallet standard
			const signMessageFeature = currentWallet.features['rtd:signMessage'];
