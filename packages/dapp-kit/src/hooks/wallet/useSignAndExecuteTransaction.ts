...
import { signTransaction } from 'rtd-wallet-standard';
...
type UseSignAndExecuteTransactionArgs = PartialBy<
	Omit<RtdSignAndExecuteTransactionInput, 'transaction'>,
	'account' | 'chain'
> & {
	transaction: Transaction | string;
};

type UseSignAndExecuteTransactionResult = RtdSignAndExecuteTransactionOutput;
...
> {
	...
	return useMutation({
		...
		mutationFn: async ({ transaction, ...signTransactionArgs }): Promise<Result> => {
			...
			if (
				!currentWallet.features['rtd:signTransaction'] &&
				!currentWallet.features['rtd:signTransactionBlock']
			) {
			...
			const chain = signTransactionArgs.chain ?? `rtd:${network}`;
