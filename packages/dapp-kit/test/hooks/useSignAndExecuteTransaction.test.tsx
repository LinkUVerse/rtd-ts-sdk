...
import { rtdFeatures } from '../mocks/mockFeatures.js';
import { createWalletProviderContextWrapper, registerMockWallet } from '../test-utils.js';
import { toBase58 } from 'rtd-utils';
...
describe('useSignAndExecuteTransaction', () => {
	...
	test('throws an error when trying to sign and execute a transaction without a wallet connection', async () => {
		...
		result.current.mutate({ transaction: new Transaction(), chain: 'rtd:testnet' });
	...
	test('signing and executing a transaction from the currently connected account works successfully', async () => {
		const { unregister, mockWallet } = registerMockWallet({
			walletName: 'Mock Wallet 1',
			features: rtdFeatures,
		});

		const rtdClient = new RtdClient({ url: getFullnodeUrl('localnet') });
		const mockSignTransactionFeature = mockWallet.features['rtd:signTransaction'];
		...
		const reportEffectsFeature = mockWallet.features['rtd:reportTransactionEffects'];
		const reportEffects = reportEffectsFeature!.reportTransactionEffects as Mock;

		reportEffects.mockImplementation(async () => {});

		const executeTransaction = vi.spyOn(rtdClient, 'executeTransactionBlock');
		...
		expect(call[0].chain).toBe('rtd:testnet');
	...
	test('executing with custom data resolver', async () => {
		...
		const rtdClient = new RtdClient({ url: getFullnodeUrl('localnet') });
		const mockSignMessageFeature = mockWallet.features['rtd:signTransaction'];
		...
		const signTransactionFeature = mockWallet.features['rtd:signTransaction'];
