...
import { signMessageFeature, rtdFeatures } from '../mocks/mockFeatures.js';
...
describe('useSignPersonalMessage', () => {
	...
	test('falls back to the `rtd:signMessage` feature with a wallet that lacks support for `rtd:signPersonalMessage`.', async () => {
		const { unregister, mockWallet } = registerMockWallet({
			walletName: 'Mock Wallet 1',
		...
		const mockSignMessageFeature = mockWallet.features['rtd:signMessage'];
	...
	test('signing a personal message from the currently connected account works successfully', async () => {
		...
		const signPersonalMessageFeature = mockWallet.features['rtd:signPersonalMessage'];
		...
		result.current.signPersonalMessage.mutate({ message, chain: 'rtd:testnet' });
