...
To install, add the [`rtd-bcs`](https://www.npmjs.com/package/rtd-bcs) package to your project:
...
```ts
import { bcs, fromHex, toHex } from 'rtd-bcs';
...
The `rtd-bcs` library can be used to define schemas that can serialize and deserialize BCS encoded
...
```ts
// Example: Generics
import { bcs, BcsType } from 'rtd-bcs';
...
transforms can also handle more complex types. For instance, `rtd-typescript` uses the following
...
```ts
import { bcs, fromBase58, fromBase64, fromHex } from 'rtd-bcs';
...
```ts
import { bcs, type InferBcsType, type InferBcsInput } from 'rtd-bcs';
