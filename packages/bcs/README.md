...
To install, add the [`@linku/bcs`](https://www.npmjs.com/package/@linku/bcs) package to your
project:
...
```ts
import { bcs, fromHex, toHex } from '@linku/bcs';
...
The `@linku/bcs` library can be used to define schemas that can serialize and deserialize BCS
...
```ts
// Example: Generics
import { bcs, BcsType } from '@linku/bcs';
...
```ts
import { bcs, toHex } from '@linku/bcs';
...
transforms can also handle more complex types. For instance, `@linku/rtd` uses the following
...
```ts
import { bcs, fromBase58, fromBase64, fromHex } from '@linku/bcs';
...
```ts
import { bcs, type InferBcsType, type InferBcsInput } from '@linku/bcs';
