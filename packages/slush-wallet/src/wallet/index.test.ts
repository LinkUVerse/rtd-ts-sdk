...
it('preserves the advertised feature subset', () => {
	expect(wallet(['rtd:signPersonalMessage']).accounts[0].features).toEqual([
		'rtd:signPersonalMessage',
	]);
});
it('keeps existing sessions compatible when no feature field was supplied', () => {
	expect(wallet().accounts[0].features).toContain('rtd:signTransaction');
});

it('only advertises features implemented by the wallet', () => {
	const instance = wallet(['rtd:signAndExecuteTransactionBlock', 'rtd:signTransaction']);
...
it('refreshes accounts when another tab replaces or clears the session', () => {
	let stored: string | null = session(['rtd:signTransaction']);
