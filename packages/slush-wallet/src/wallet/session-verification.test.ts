...
import { createJwtSession, WalletPostMessageChannel } from 'rtd-window-wallet-core';
...
it('allows the explicitly included request and legacy sessions', async () => {
	await expect(
		(await request('sign-personal-message', ['rtd:signPersonalMessage'])).verifyJwtSession(key),
	).resolves.toBeTruthy();
...
it('does not widen a restricted session to a different request type', async () => {
	await expect(
		(await request('sign-transaction', ['rtd:signPersonalMessage'])).verifyJwtSession(key),
	).rejects.toThrow('not authorized');
	await expect(
		(await request('sign-transaction', ['rtd:signTransactionBlock'])).verifyJwtSession(key),
	).resolves.toBeTruthy();
});
