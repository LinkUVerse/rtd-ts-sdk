...
import { fromBase64, toBase64, toHex } from '@linku/bcs';
...
import { RtdGraphQLClient } from '../graphql/client.js';
...
import type { ClientWithExtensions, Experimental_RtdClientTypes } from '../experimental/types.js';

export interface ZkLoginCompatibleClient extends ClientWithExtensions<{
	core: {
		verifyZkLoginSignature: Experimental_RtdClientTypes.TransportMethods['verifyZkLoginSignature'];
	};
}> {}
...
export class ZkLoginPublicIdentifier extends PublicKey {
	...
	static fromBytes(
		...
		if (legacyAddress === true) {
			...
			if (publicKey.toRtdAddress() !== address) {
				publicKey = new ZkLoginPublicIdentifier(normalizeZkLoginPublicKeyBytes(bytes, true), {
					client,
				});
			}
		...
		if (address && publicKey.toRtdAddress() !== address) {
	...
	static fromProof(address: string, proof: ZkLoginSignatureInputs) {
		...
		if (legacyPublicKey.toRtdAddress() === address) {
	...
	override toRtdAddress(): string {
		if (this.#legacyAddress) {
			return this.#toLegacyAddress();
		}

		return super.toRtdAddress();
	}
	...
	#toLegacyAddress() {
		...
		return normalizeRtdAddress(
			bytesToHex(blake2b(addressBytes, { dkLen: 32 })).slice(0, RTD_ADDRESS_LENGTH * 2),
		);
	}
	...
	/**
	 * Return the Rtd address associated with this ZkLogin public identifier
	 */
	flag(): number {
	...
	verifyPersonalMessage(message: Uint8Array, signature: Uint8Array | string): Promise<boolean> {
		...
		const address = new ZkLoginPublicIdentifier(parsedSignature.publicKey).toRtdAddress();
	...
	override verifyAddress(address: string): boolean {
		return address === super.toRtdAddress() || address === this.#toLegacyAddress();
	}
}
...
async function graphqlVerifyZkLoginSignature({
	...
	signature,
	intentScope,
	client = new RtdGraphQLClient({
		url: 'https://graphql.mainnet.rtd.life/graphql',
	}),
}: {
	address: string;
	bytes: string;
