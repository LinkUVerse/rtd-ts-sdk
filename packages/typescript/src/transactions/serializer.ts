...
import { MOVE_STDLIB_ADDRESS, RTD_FRAMEWORK_ADDRESS } from '../utils/index.js';
...
export function isTxContext(param: RtdClientTypes.OpenSignature): boolean {
	...
	return (
		normalizeRtdAddress(pkg) === RTD_FRAMEWORK_ADDRESS &&
		module === 'tx_context' &&
		name === 'TxContext'
	);
}
...
export function getPureBcsSchema(
	typeSignature: RtdClientTypes.OpenSignatureBody,
): BcsType<any> | null {
	switch (typeSignature.$kind) {
		...
		case 'datatype': {
			...
			const normalizedPkg = normalizeRtdAddress(pkg);
			...
			if (normalizedPkg === RTD_FRAMEWORK_ADDRESS) {
