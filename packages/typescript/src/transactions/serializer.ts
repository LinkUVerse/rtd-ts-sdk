...
export function isTxContext(param: OpenMoveTypeSignature): boolean {
	...
	return (
		!!struct &&
		normalizeRtdAddress(struct.package) === normalizeRtdAddress('0x2') &&
...
export function getPureBcsSchema(typeSignature: OpenMoveTypeSignatureBody): BcsType<any> | null {
	...
	if ('datatype' in typeSignature) {
		const pkg = normalizeRtdAddress(typeSignature.datatype.package);

		if (pkg === normalizeRtdAddress(MOVE_STDLIB_ADDRESS)) {
			if (
		...
		if (
			pkg === normalizeRtdAddress(RTD_FRAMEWORK_ADDRESS) &&
