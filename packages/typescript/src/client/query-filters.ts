...
export async function resolveTransactionFilter(
	...
	return {
		$kind: 'function',
		package: normalizeRtdAddress((await mvr.resolvePackage({ package: pkg, signal })).package),
		module,
		function: fn,
	};
}
...
export async function resolveEventFilter(
	mvr: RtdClientTypes.MvrMethods,
	filter: RtdClientTypes.EventFilter,
	signal?: AbortSignal,
