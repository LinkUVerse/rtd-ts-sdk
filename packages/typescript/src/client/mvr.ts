...
export class MvrClient implements RtdClientTypes.MvrMethods {
	#cache: ClientCache;
	#url?: string;
	#pageSize: number;
	#overrides: {
	...
	async resolvePackage({
		package: name,
		signal,
	}: RtdClientTypes.MvrResolvePackageOptions): Promise<RtdClientTypes.MvrResolvePackageResponse> {
	...
	async resolveType({
		type,
		signal,
	}: RtdClientTypes.MvrResolveTypeOptions): Promise<RtdClientTypes.MvrResolveTypeResponse> {
		signal?.throwIfAborted();
		if (!hasMvrName(type)) {
			return {
				type,
	...
	async resolve({
		...
		packages = [],
		signal,
	}: RtdClientTypes.MvrResolveOptions): Promise<RtdClientTypes.MvrResolveResponse> {
		signal?.throwIfAborted();
...
export function replaceNames(
	...
	resolved: RtdClientTypes.MvrResolveResponse,
) {
	for (const command of builder.commands) {
