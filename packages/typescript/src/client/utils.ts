...
function parseBcsExecutionError(failure: {
	...
	switch (error.$kind) {
		...
		case 'RtdMoveVerificationTimedout':
...
): RtdClientTypes.TransactionData {
	return (
		onlyTransactionKind
...
export function parseTransactionEffectsBcs(effects: Uint8Array): RtdClientTypes.TransactionEffects {
...
}): RtdClientTypes.TransactionEffects {
...
function parseTransactionEffectsV2({
	...
	const changedObjects = effects.changedObjects.map(
		([id, change]): RtdClientTypes.ChangedObject => {
			return {
				objectId: id,
	...
	return {
		...
		unchangedConsensusObjects: effects.unchangedConsensusObjects.map(
			([objectId, object]): RtdClientTypes.UnchangedConsensusObject => {
				return {
					kind: object.$kind,
					objectId: objectId,
					version:
