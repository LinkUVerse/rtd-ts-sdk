...
export async function coreClientResolveTransactionPlugin(
	...
	const normalizedGasPayer = gasPayer ? normalizeRtdAddress(gasPayer) : null;
	for (const input of transactionData.inputs) {
		if (input.$kind !== 'FundsWithdrawal' || !normalizedGasPayer) continue;
		if (normalizeStructTag(input.FundsWithdrawal.typeArg.Balance) !== RTD_TYPE_ARG) continue;
		...
		if (
			withdrawalOwner &&
			normalizeRtdAddress(withdrawalOwner) === normalizedGasPayer &&
	...
	const [, systemStateResult, balanceResult, coinsResult, chainIdResult] = await Promise.all([
		...
		needsPayment && gasPayer
			? client.core.listCoins({ owner: gasPayer, coinType: RTD_TYPE_ARG })
			: null,
...
async function setGasBudget(
	...
	} as RtdClientTypes.SimulateTransactionOptions<{ effects: true }> & { doGasSelection: boolean });
...
function setGasPayment({
	...
	transactionData: TransactionDataBuilder;
	balance: RtdClientTypes.GetBalanceResponse;
	coins: RtdClientTypes.ListCoinsResponse;
	usesGasCoin: boolean;
	withdrawals: bigint;
	gasPayer: string;
...
async function resolveObjectReferences(
	...
	const dedupedIds = [
		...new Set(
			objectsToResolve.map((input) => normalizeRtdObjectId(input.UnresolvedObject.objectId)),
		),
	];
	...
	for (const [index, input] of transactionData.inputs.entries()) {
		...
		const id = normalizeRtdAddress(input.UnresolvedObject.objectId);
...
async function normalizeInputs(transactionData: TransactionDataBuilder, client: ClientWithCoreApi) {
	...
	const moveFunctionParameters = new Map<string, RtdClientTypes.OpenSignature[]>();
...
function isReceivingType(type: RtdClientTypes.OpenSignature): boolean {
