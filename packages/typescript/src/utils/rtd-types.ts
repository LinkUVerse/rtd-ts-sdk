...
// Source of truth is
// https://github.com/LinkUVerse/rtd/blob/acb2b97ae21f47600e05b0d28127d88d0725561d/crates/rtd-types/src/base_types.rs#L67
// which uses the Move account address length
// https://github.com/move-language/move/blob/67ec40dc50c66c34fd73512fcc412f3b68d67235/language/move-core/types/src/account_address.rs#L23 .

export const RTD_ADDRESS_LENGTH = 32;
export function isValidRtdAddress(value: string): value is string {
	return isHex(value) && getHexByteLength(value) === RTD_ADDRESS_LENGTH;
}

export function isValidRtdObjectId(value: string): boolean {
...
export function parseStructTag(type: string): StructTag {
	...
	return {
		address: isMvrPackage ? address : normalizeRtdAddress(address),
		module,
		name,
		typeParams,
	};
 ...
 * 3. Add more zeros if the length of the address(excluding `0x`) is less than `RTD_ADDRESS_LENGTH`
 *
 ...
 *
 */
export function normalizeRtdAddress(value: string, forceAdd0x: boolean = false): string {
	let address = value.toLowerCase();
	if (!forceAdd0x && address.startsWith('0x')) {
		address = address.slice(2);
	}
	return `0x${address.padStart(RTD_ADDRESS_LENGTH * 2, '0')}`;
}

export function normalizeRtdObjectId(value: string, forceAdd0x: boolean = false): string {
	return normalizeRtdAddress(value, forceAdd0x);
}
