...
/**
 * @deprecated "Use requestRtdFromFaucetV2 instead"
 */
export async function requestRtdFromFaucetV0(input: {
	host: string;
	recipient: string;
	headers?: HeadersInit;
 ...
 * @deprecated "Use requestRtdFromFaucetV2 instead"
 */
export async function requestRtdFromFaucetV1(input: {
	host: string;
	recipient: string;
	headers?: HeadersInit;
...
export async function requestRtdFromFaucetV2(input: {
	host: string;
	recipient: string;
	headers?: HeadersInit;
}) {
...
/**
 * @deprecated "Use requestRtdFromFaucetV2 which returns directly a success or failure status"
 */
...
export function getFaucetHost(network: 'testnet' | 'devnet' | 'localnet') {
	switch (network) {
		case 'testnet':
			return 'https://faucet.testnet.rtd.life';
		case 'devnet':
			return 'https://faucet.devnet.rtd.life';
		case 'localnet':
