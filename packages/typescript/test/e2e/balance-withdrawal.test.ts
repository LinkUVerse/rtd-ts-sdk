...
describe('Balance Withdrawal', () => {
	...
	it('deposits RTD to address balance and withdraws it', async () => {
		const depositAmount = 200_000_000n; // 0.2 RTD
		...
		const withdrawalAmount = 100_000_000n; // 0.1 RTD — less than deposit to leave room for gas
	...
	it('uses address balance for gas payment and withdrawal', async () => {
		const depositAmount = 200_000_000n; // 0.2 RTD (enough for gas + withdrawal)
		const withdrawalAmount = 50_000_000n; // 0.05 RTD

		// Step 1: Deposit RTD to address balance
		const depositTx = new Transaction();
		const [coinToDeposit] = depositTx.splitCoins(depositTx.gas, [depositAmount]);
		...
		const [genesisCheckpoint, gasPrice, systemState] = await Promise.all([
			...
			toolbox.jsonRpcClient.getLatestRtdSystemState(),
		]);
		...
		tx.setGasBudget(100_000_000n); // 0.1 RTD budget
		tx.setExpiration({
			ValidDuring: {
