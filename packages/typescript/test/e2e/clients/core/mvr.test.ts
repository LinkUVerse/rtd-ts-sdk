...
import { RtdJsonRpcClient, JsonRpcHTTPTransport } from '../../../../src/jsonRpc/index.js';
...
describe('Core API - MVR (Move Registry)', () => {
	...
	beforeAll(async () => {
		...
		const testnetJsonRpcUrl = 'https://fullnode.testnet.rtd.life';
		const testnetGrpcUrl = 'https://fullnode.testnet.rtd.life:443';

		jsonRpcClient = new RtdJsonRpcClient({
			network: 'testnet',
			transport: new JsonRpcHTTPTransport({
				url: testnetJsonRpcUrl,
			}),
		});
		grpcClient = new RtdGrpcClient({
			network: 'testnet',
			baseUrl: testnetGrpcUrl,
		});
		graphqlClient = new RtdGraphQLClient({
			network: 'testnet',
			url: 'https://graphql.testnet.rtd.io/graphql',
		});
	});
	...
	describe('MVR Type Resolution', () => {
		...
		testWithAllClients('should resolve generic type with named package', async (client) => {
			const typeString = `${deepbookPackage}::pool::Pool<0x2::rtd::RTD>`;
		...
		testWithAllClients('should resolve type without MVR name (passthrough)', async (client) => {
			...
			const typeString = `0x2::coin::Coin<0x2::rtd::RTD>`;
