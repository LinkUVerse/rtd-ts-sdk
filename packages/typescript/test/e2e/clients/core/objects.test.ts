...
import { normalizeRtdAddress, RTD_TYPE_ARG } from '../../../../src/utils/index.js';
...
describe('Core API - Objects', () => {
	...
	describe('getObject', () => {
		...
		testWithAllClients(
			...
			async (client) => {
				...
				expect(object.objectId).toBe(normalizeRtdAddress(testObjectId));
		...
		testWithAllClients('should throw error for non-existent object', async (client) => {
			const fakeObjectId = normalizeRtdAddress('0x9999');
	...
	describe('getObjects', () => {
		testWithAllClients('should get multiple objects', async (client) => {
			...
			for (let i = 0; i < objects.length; i++) {
				...
				if (!(result instanceof Error)) {
					expect(result.objectId).toBe(normalizeRtdAddress(coinIds[i]));
		...
		testWithAllClients('should handle mix of valid and invalid object IDs', async (client) => {
			const objectIds = [testObjectId, normalizeRtdAddress('0x9999')];
	...
	describe('listOwnedObjects', () => {
		...
		testWithAllClients('should filter owned objects by type', async (client) => {
			// Filter by RTD coin type
			const result = await client.core.listOwnedObjects({
				owner: testAddress,
				type: `0x2::coin::Coin<${RTD_TYPE_ARG}>`,
			});

			expect(result.objects.length).toBeGreaterThan(0);

			// Verify all objects are RTD coins
			for (const obj of result.objects) {
				expect(obj.type).toContain('Coin<');
				expect(obj.type).toContain('rtd::RTD');
			}
		});
		...
		testWithAllClients('should return empty array for address with no objects', async (client) => {
			const emptyAddress = Ed25519Keypair.generate().getPublicKey().toRtdAddress();
			const result = await client.core.listOwnedObjects({
				owner: emptyAddress,
			});
	...
	describe('getObject - Package Objects', () => {
		testWithAllClients('should fetch package object correctly', async (client) => {
			...
			expect(object.objectId).toBe(normalizeRtdAddress(testPackageId));
