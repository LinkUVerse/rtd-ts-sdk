...
import { RtdGrpcClient, GrpcWebFetchTransport } from '../../../../src/grpc/index.js';
...
/**
 * E2E tests verifying that RtdGrpcClient works correctly with both the default
...
describe('gRPC transport variants', () => {
	...
	let nativeGrpcClient: RtdGrpcClient | undefined;
	let grpcWebCustomClient: RtdGrpcClient;
	...
	beforeAll(async () => {
		...
		if (GrpcTransport && ChannelCredentials) {
			...
			nativeGrpcClient = new RtdGrpcClient({
				network: 'localnet',
		...
		grpcWebCustomClient = new RtdGrpcClient({
			network: 'localnet',
	...
	describe('getBalance', () => {
		it.skipIf(!GrpcTransport)('native gRPC transport returns balance', async () => {
			...
			expect(balance.coinType).toContain('rtd::RTD');
