...
describe('Core API - Name Service', () => {
	...
	describe('defaultNameServiceName', () => {
		// Note: RtdNS is not configured on localnet, so we expect specific behavior
		// gRPC throws "RtdNS not configured for this network"
		...
		testWithAllClients('should handle name service request', async (client) => {
			try {
				...
				// On localnet without RtdNS, name will be null or undefined
				if (result.data.name !== null && result.data.name !== undefined) {
					expect(typeof result.data.name).toBe('string');
				}
			} catch (error) {
				// gRPC and GraphQL may throw errors when RtdNS is not configured
