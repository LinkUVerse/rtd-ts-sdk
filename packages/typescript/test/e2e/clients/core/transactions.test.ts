...
describe('Core API - Transactions', () => {
	...
	describe('simulateTransaction', () => {
		...
		testWithAllClients('should not actually execute transaction', async (client) => {
			...
			tx.transferObjects(
				...
				tx.pure.address(Ed25519Keypair.generate().toRtdAddress()),
			);
	...
	describe('simulateTransaction - gas selection', () => {
		testWithAllClients(
			...
			async (client) => {
				// Sender's RTD is held entirely in an address balance (no coin objects), and the
				...
				const tx = new Transaction();
				const withdrawal = tx.withdrawal({ amount: 100_000_000n, type: RTD_TYPE_ARG });
				const balance = tx.moveCall({
					target: '0x2::balance::redeem_funds',
					typeArguments: [RTD_TYPE_ARG],
		...
		testWithAllClients(
			...
			async (client) => {
				const coins = await client.core.listCoins({ owner: testAddress, coinType: RTD_TYPE_ARG });
	...
	describe('objectTypes', () => {
		...
		testWithAllClients('should return object types for coin operations', async (client) => {
			...
			// Should have RTD coin types (address format varies by client)
			const types = Object.values(objectTypes!);
			const rtdCoinType = types.find(
				(t) => t.includes('::coin::Coin<') && t.includes('::rtd::RTD>'),
			);
			expect(rtdCoinType).toBeDefined();
		});
