...
const ALLOWANCE_TYPE = '0x2::balance::Balance<0x2::rtd::RTD>';
const LIFETIME_CAP = 1_000_000_000n; // 1 RTD
const SPEND_AMOUNT = 10_000_000n; // 0.01 RTD
...
describe('Allowance withdrawals', () => {
	...
	function buildSpend() {
		...
		const balance = tx.balance({ allowance: allowanceId, balance: SPEND_AMOUNT, type: RTD });
	...
	async function issueAllowance(issuer: Ed25519Keypair, spenderAddress: string, app = false) {
		// The funder issues an allowance over its RTD address balance to the spender.
		const tx = new Transaction();
		...
		for (const knownFunder of [false, true]) {
			testWithAllClients(
				...
				async (client) => {
					...
					tx.transferObjects([coin], spender.address);
					const before = await client.core.getBalance({ owner: funder.address, coinType: RTD });
					const result = await client.core.signAndExecuteTransaction({
						transaction: tx,
						signer: spender.keypair,
					});
					...
					await toolbox.waitForTransaction({ digest: result.Transaction!.digest });
					const after = await client.core.getBalance({ owner: funder.address, coinType: RTD });
