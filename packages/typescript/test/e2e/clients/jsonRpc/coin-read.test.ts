...
// See: rtd/crates/rtd-types/src/coin_reservation.rs
...
describe('CoinRead API', () => {
	...
	it('Get coins with/without type', async () => {
		const rtdCoins = await toolbox.jsonRpcClient.getCoins({
		...
		const someRtdCoins = await toolbox.jsonRpcClient.getCoins({
			owner: toolbox.address(),
			limit: 3,
		});
	...
	it('Get balance with/without type', async () => {
		const rtdBalance = await toolbox.jsonRpcClient.getBalance({
			owner: toolbox.address(),
		});
	...
	describe('Address Balance', () => {
		it('filters out fake address balance coins from listCoins', async () => {
			// Deposit some RTD to the address balance
