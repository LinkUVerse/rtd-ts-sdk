...
describe('Object id/Address/Transaction digest validation', () => {
	...
	it('Test all functions with invalid Rtd Address', async () => {
		//empty id
	...
	it('Test all functions with invalid Object Id', async () => {
		...
		).rejects.toThrowError(/Invalid Rtd Object id/);
	...
	it('Validates tx.pure.address and tx.pure.id', async () => {
		...
		expect(() => tx.pure.address('')).toThrowError(/Invalid Rtd address/);
		expect(() => tx.pure.id('')).toThrowError(/Invalid Rtd address/);
	});
});
