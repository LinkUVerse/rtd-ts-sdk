...
use std::internal;
use rtd::allowance::{Self, AllowanceProposal, SpendPermit};
...
public fun spend_and_send<C>(
    allowance: &mut allowance::Allowance<rtd::balance::Balance<C>>,
    withdrawal: allowance::AllowanceWithdrawal<rtd::balance::Balance<C>>,
    clock: &rtd::clock::Clock,
    recipient: address,
    ctx: &TxContext,
) {
    let balance = allowance::balance_spend(allowance, withdrawal, clock, ctx);
    rtd::balance::send_funds(balance, recipient);
}
