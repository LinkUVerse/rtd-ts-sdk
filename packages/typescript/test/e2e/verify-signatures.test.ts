	...
	describe('personal message signatures', () => {
		describe('single signatures', () => {
			describe('Ed25519', () => {
				...
				it('verifies valid signatures', async () => {
					...
					expect(publicKey.toRtdAddress()).toBe(address);
				});
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Ed25519Keypair().getPublicKey().toRtdAddress();
					await expect(
			...
			describe('Secp256k1', () => {
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Secp256k1Keypair().getPublicKey().toRtdAddress();
					await expect(
						verifyPersonalMessageSignature(message, signature, { address: wrongAddress }),
					).rejects.toThrow();
				});
			...
			describe('Secp256r1', () => {
				...
				it('fails for wrong address', async () => {
					const { signature } = await keypair.signPersonalMessage(message);
					const wrongAddress = new Secp256r1Keypair().getPublicKey().toRtdAddress();
					await expect(
						verifyPersonalMessageSignature(message, signature, { address: wrongAddress }),
					).rejects.toThrow();
				});
		...
		describe('multisig signatures', () => {
			...
			it('verifies valid multisig signatures', async () => {
				...
				expect(publicKey.toRtdAddress()).toBe(multiSigPublicKey.toRtdAddress());
			});
		...
		describe('zkLogin signatures', () => {
			...
			it('verifies valid signatures', async () => {
				...
				expect(publicKey.toRtdAddress()).toBe(
