...
import { ClientWithCoreApi, RtdClientTypes } from '../../src/client/index.js';

function getOwnerAddress(o: RtdClientTypes.ChangedObject): string | undefined {
	return (
...
describe('Transfer to Object', () => {
	...
	let parentObjectId: RtdClientTypes.ChangedObject;
	let receiveObjectId: RtdClientTypes.ChangedObject;
	...
	beforeEach(async () => {
		...
		const y = x.Transaction?.effects?.changedObjects
			.filter((o: RtdClientTypes.ChangedObject) => o.idOperation === 'Created')
			.map((o: RtdClientTypes.ChangedObject) => getOwnerAddress(o))!;
		receiveObjectId = x.Transaction?.effects?.changedObjects!.filter(
			(o: RtdClientTypes.ChangedObject) =>
		...
		const sharedObject = x.Transaction?.effects?.changedObjects!.filter(
			(o: RtdClientTypes.ChangedObject) => getOwnerAddress(o) === undefined,
		)[0]!;
