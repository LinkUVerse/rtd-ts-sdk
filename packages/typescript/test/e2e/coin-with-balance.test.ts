...
import { fromHex, toBase64 } from '@linku/bcs';
...
describe('coinWithBalance', () => {
	...
	it('works with rtd', async () => {
		...
		tx.setSender(publishToolbox.keypair.toRtdAddress());
		...
		expect(
			...
			sender: publishToolbox.keypair.toRtdAddress(),
			commands: [
				{
					$Intent: {
						data: {
