...
describe('filter resolution abort signals', () => {
	it('forwards the abort signal to MVR resolution', async () => {
		...
		const recordingMvr: RtdClientTypes.MvrMethods = {
...
const mvr: RtdClientTypes.MvrMethods = {
	resolvePackage: async ({ package: pkg }) => ({
		package: pkg === 'rtd-demo' ? '0x123' : pkg,
	}),
	resolveType: async ({ type }) => ({ type: type.replaceAll('rtd-demo', '0x123') }),
	resolve: async () => ({ packages: {}, types: {} }),
};
...
describe('resolveTransactionFilter', () => {
	...
	it('resolves MVR names in function paths', async () => {
		expect(await resolveTransactionFilter(mvr, { function: 'rtd-demo::foo::bar' })).toEqual({
			$kind: 'function',
	...
	it('throws unless exactly one predicate is specified', async () => {
		await expect(
			resolveTransactionFilter(mvr, {} as RtdClientTypes.TransactionFilter),
...
describe('resolveEventFilter', () => {
	...
	it('resolves MVR names in eventType filters', async () => {
		expect(await resolveEventFilter(mvr, { eventType: 'rtd-demo::foo::Bar' })).toEqual({
			$kind: 'eventType',
	...
	it('throws unless exactly one predicate is specified', async () => {
		await expect(resolveEventFilter(mvr, {} as RtdClientTypes.EventFilter)).rejects.toThrowError(
