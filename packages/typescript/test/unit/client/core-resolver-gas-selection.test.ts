...
import { RtdJsonRpcClient } from '../../../src/jsonRpc/client.js';
...
const SENDER = normalizeRtdAddress('0xabc');
const CHAIN = toBase58(new Uint8Array(32).fill(1));
const DIGEST = toBase58(new Uint8Array(32).fill(2));

function ref(id: number) {
	return { objectId: normalizeRtdAddress('0x' + id.toString(16)), version: '1', digest: DIGEST };
}

function coins(count: number, start = 1): RtdClientTypes.Coin[] {
...
function paymentRefs(objects: RtdClientTypes.Coin[]) {
...
function createClient(objects = coins(300), addressBalance = '0') {
	const client = new RtdJsonRpcClient({ network: 'testnet', url: 'http://localhost:9000' });
	...
	} as RtdClientTypes.GetCurrentSystemStateResponse);
...
describe('bounded automatic gas selection', () => {
	...
	it('does not reserve a slot when explicit withdrawals use the entire address balance', async () => {
		const tx = transaction();
		tx.splitCoins(tx.gas, [1]);
		tx.withdrawal({ amount: 100, type: RTD_TYPE_ARG });
