...
import { RtdJsonRpcClient } from '../../../src/jsonRpc/index.js';
...
describe('core transaction metadata', () => {
	...
	it('maps checkpoint and timestamp from JSON-RPC ledger reads', async () => {
		const transport: JsonRpcTransport = {
			async request<T>(input: JsonRpcTransportRequestOptions) {
				expect(input.method).toBe('rtd_getTransactionBlock');
				return {
					digest: DIGEST,
					effects: {
		...
		const client = new RtdJsonRpcClient({ network: 'mainnet', transport });
	...
	it('returns null checkpoint and timestamp from JSON-RPC execution', async () => {
		const transport: JsonRpcTransport = {
			async request<T>(input: JsonRpcTransportRequestOptions) {
				expect(input.method).toBe('rtd_executeTransactionBlock');
				return {
					digest: DIGEST,
					effects: {
						status: { status: 'success' },
