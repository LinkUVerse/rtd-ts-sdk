...
import { ObjectError, RtdClientError, TransactionError } from '../../../src/client/index.js';
...
describe('client lookup errors', () => {
	it('keeps the existing ObjectError constructor without guessing a normalized reason', () => {
		...
		expect(error).toBeInstanceOf(RtdClientError);
...
describe('getObjects', () => {
	it.each([
		...
		const client = new RtdGrpcClient({ network: 'testnet', baseUrl: 'http://localhost' });
	...
	it.each([
		...
		const client = new RtdJsonRpcClient({ network: 'testnet', transport });
