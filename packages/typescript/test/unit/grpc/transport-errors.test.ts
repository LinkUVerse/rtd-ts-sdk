...
import { GrpcWebFetchTransport, RtdGrpcClient } from '../../../src/grpc/index.js';
...
describe('a caller-supplied transport', () => {
	...
	it('resolves an expired name reported by another copy of runtime-rpc', async () => {
		...
		await expect(client.core.resolveNameServiceAddress({ name: '@linku' })).resolves.toEqual({
			address: null,
		});
	});
	...
	it('honours the decoded marker set by another copy of this package', async () => {
		...
		Object.defineProperty(decoded, Symbol.for('rtd-typescript/grpc/decoded-status-message'), {
			value: true,
		});
		const client = new RtdGrpcClient({
			network: 'mainnet',
		...
		await expect(client.core.resolveNameServiceAddress({ name: '@linku' })).rejects.toBe(decoded);
	});
	...
	it('does not decode a status the transport already decoded', async () => {
		...
		const error = await captureError(client.core.resolveNameServiceAddress({ name: '@linku' }));
...
describe('gRPC transport call options', () => {
	it("gives every call its own options rather than writing to the transport's", async () => {
		...
		const client = new RtdGrpcClient({ network: 'testnet', transport });
