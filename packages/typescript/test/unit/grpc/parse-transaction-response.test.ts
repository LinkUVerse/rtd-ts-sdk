...
import type { RtdClientTypes } from '../../../src/client/index.js';
import {
	GrpcTypes,
...
describe('gRPC transaction response parsers', () => {
	it('parses protobuf JSON simulation responses into SDK simulation results', () => {
		...
		expectTypeOf(result.Transaction.events).toEqualTypeOf<RtdClientTypes.Event[]>();
		expectTypeOf(result.Transaction.objectTypes).toEqualTypeOf<Record<string, string>>();
		expectTypeOf(result.commandResults).toEqualTypeOf<RtdClientTypes.CommandResult[]>();
		expect(result.Transaction.objectTypes).toEqual({
			'0x2': '0x2::coin::Coin<0x2::rtd::RTD>',
		});
		...
		if (transactionResult.$kind === 'Transaction') {
			...
			).toEqualTypeOf<RtdClientTypes.TransactionEffects>();
