...
import { fromBase64, toBase58 } from '@linku/bcs';
...
// Test case generated against rust keytool cli. See https://github.com/LinkUVerse/rtd/blob/edd2cd31e0b05d336b1b03b6e79a67d8dd00d06b/crates/rtd/src/unit_tests/keytool_tests.rs#L165
const TEST_CASES = [
	[
...
describe('ed25519-keypair', () => {
	...
	it('create keypair from secret key and mnemonics matches keytool', () => {
		for (const t of TEST_CASES) {
			...
			expect(keypair.getPublicKey().toRtdAddress()).toEqual(t[2]);

			// Decode Rtd private key from Bech32 string
			const parsed = decodeRtdPrivateKey(t[1]);
			const kp = Ed25519Keypair.fromSecretKey(parsed.secretKey);
			expect(kp.getPublicKey().toRtdAddress()).toEqual(t[2]);
	...
	it('signs Transactions', async () => {
		...
		tx.setSender(keypair.getPublicKey().toRtdAddress());
		tx.setGasPrice(5);
		tx.setGasBudget(100);
		tx.setGasPayment([
			{
