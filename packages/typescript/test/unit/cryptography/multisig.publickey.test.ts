...
import { normalizeRtdAddress } from '../../../src/utils/rtd-types.js';
...
describe('Publickey', () => {
	...
	it('`toRtdAddress()` should return correct rtd address associated with multisig publickey', async () => {
		const multiSigPublicKey = MultiSigPublicKey.fromPublicKeys({
			threshold: 3,
			publicKeys: [
				{ publicKey: pk1, weight: 1 },
		...
		for (const { publicKey, weight } of multiSigPublicKey.getPublicKeys()) {
			const bytes = publicKey.toRtdBytes();
			tmp.set(bytes, i);
			i += bytes.length;
			tmp.set([weight], i++);
		}
		const multisigRtdAddress = normalizeRtdAddress(
			bytesToHex(blake2b(tmp.slice(0, i), { dkLen: 32 })),
		);

		expect(multiSigPublicKey.toRtdAddress()).toEqual(multisigRtdAddress);
		expect(multiSigPublicKey.toRtdAddress()).toEqual(
