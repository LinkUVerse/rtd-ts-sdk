...
import { normalizeRtdAddress, RTD_ADDRESS_LENGTH } from '../../../src/utils/rtd-types.js';
...
describe('Publickey', () => {
	...
	it('`toRtdPublicKey()` should return a valid rtd representation', async () => {
		expect(pk2.toRtdPublicKey()).toEqual(toBase64(pk2.toRtdBytes()));
		expect(pk2.toRtdPublicKey()).toEqual('AQIdFSMHxrcrDtBBiw5wzYDn9SlbjYb1ci0/UhP70jlPNg==');
	});
	...
	it('`toRtdBytes()` should return the correct byte representation of the public key with the signature scheme flag', async () => {
		const pk1RtdBytes = new Uint8Array(pk1.toRawBytes().length + 1);
		pk1RtdBytes.set([0x00]);
		pk1RtdBytes.set(pk1.toRawBytes(), 1);

		expect(pk1.toRtdBytes()).toEqual(pk1RtdBytes);
		expect(pk1.toRtdBytes()).toEqual(
			new Uint8Array([
		...
		const pk2RtdBytes = new Uint8Array(pk2.toRawBytes().length + 1);
		pk2RtdBytes.set([0x01]);
		pk2RtdBytes.set(pk2.toRawBytes(), 1);

		expect(pk2.toRtdBytes()).toEqual(pk2RtdBytes);
		expect(pk2.toRtdBytes()).toEqual(
			new Uint8Array([
		...
		const pk3RtdBytes = new Uint8Array(pk3.toRawBytes().length + 1);
		pk3RtdBytes.set([0x02]);
		pk3RtdBytes.set(pk3.toRawBytes(), 1);

		expect(pk3.toRtdBytes()).toEqual(pk3RtdBytes);
		expect(pk3.toRtdBytes()).toEqual(
			new Uint8Array([
	...
	it('`toRtdAddress()` should correctly return rtd address associated with Ed25519 publickey', async () => {
		const pk1RtdAddress = normalizeRtdAddress(
			bytesToHex(blake2b(pk1.toRtdBytes(), { dkLen: 32 })).slice(0, RTD_ADDRESS_LENGTH * 2),
		);
		const pk2RtdAddress = normalizeRtdAddress(
			bytesToHex(blake2b(pk2.toRtdBytes(), { dkLen: 32 })).slice(0, RTD_ADDRESS_LENGTH * 2),
		);
		const pk3RtdAddress = normalizeRtdAddress(
			bytesToHex(blake2b(pk3.toRtdBytes(), { dkLen: 32 })).slice(0, RTD_ADDRESS_LENGTH * 2),
		);
		expect(k1.toRtdAddress()).toEqual(pk1RtdAddress);
		expect(k1.toRtdAddress()).toEqual(
			'0xafedf3bc60bd296aa6830d7c48ca44e0f7a32478ae4bd7b9a6ac1dc81ff7b29b',
		);

		expect(k2.toRtdAddress()).toEqual(pk2RtdAddress);
		expect(k2.toRtdAddress()).toEqual(
			'0x7e4f9a35bf3b5383802d990956d6f3c93e6184ebbbcf0820c124ab3a59ef77ac',
		);

		expect(k3.toRtdAddress()).toEqual(pk3RtdAddress);
		expect(k3.toRtdAddress()).toEqual(
