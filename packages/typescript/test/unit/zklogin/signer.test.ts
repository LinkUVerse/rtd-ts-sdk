...
import { bcs, fromBase64, toBase64 } from 'rtd-bcs';
...
describe('ZkLoginSigner', () => {
	...
	test('derives the current (non-legacy) address from the proof', () => {
		...
		expect(signer.getPublicKey().toRtdAddress()).toBe(expected);
		expect(signer.toRtdAddress()).toBe(expected);
	});

	test('derives the legacy address from the proof when legacyAddress is true', () => {
		const signer = makeSigner(true);
		expect(signer.getPublicKey().toRtdAddress()).toBe(anAddress);
	});
	...
	test('propagates legacyAddress into derivation (legacy and current differ for a short seed)', () => {
		...
		expect(legacy.getPublicKey().toRtdAddress()).not.toBe(current.getPublicKey().toRtdAddress());
		expect(legacy.getPublicKey().toRtdAddress()).toBe(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: true }).toRtdAddress(),
		);
		expect(current.getPublicKey().toRtdAddress()).toBe(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: false }).toRtdAddress(),
		);
	});
