...
const ID = normalizeRtdAddress('0xa11');
const FUNDER = normalizeRtdAddress('0xf00d');
const SENDER = normalizeRtdAddress('0x123');
const RTD = normalizeStructTag('0x2::rtd::RTD');
const APP = normalizeStructTag('0xa::app::APP');
const TYPE = normalizeStructTag(`0x2::allowance::Allowance<0x2::balance::Balance<${RTD}>>`);
...
describe('allowance balances', () => {
	...
	it.each([
		{ allowanceFirst: true, useGasCoin: true, type: RTD },
		{ allowanceFirst: false, useGasCoin: true, type: RTD },
		{ allowanceFirst: true, useGasCoin: false, type: RTD },
		{ allowanceFirst: false, useGasCoin: false, type: RTD },
		...
		async ({ allowanceFirst, useGasCoin, type }) => {
			...
			listCoins.mockResolvedValue({
				objects: [
					{
						objectId: normalizeRtdAddress('0xc01'),
						version: '1',
		...
		async (knownFunder) => {
			...
			const resolve = vi.fn(async ({ types }: { types: string[] }) => ({
				...
				types: Object.fromEntries(
					types.map((type) => [type, { type: type.includes('::rtd::') ? RTD : APP }]),
				),
			}));
			Object.assign(client.core, { mvr: { resolve } });
			tx.balance({
				balance: 1n,
				type: '@test/coins::rtd::RTD',
				allowance: {
					objectId: ID,
					funder: knownFunder ? FUNDER : undefined,
	...
	it.each([
		...
		[{ type: normalizeStructTag('0x2::coin::Coin<0x2::rtd::RTD>') }, /Expected a shared/],
