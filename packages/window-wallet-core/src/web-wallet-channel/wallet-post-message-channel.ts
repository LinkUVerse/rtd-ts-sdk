...
export class WalletPostMessageChannel {
	...
	async verifyJwtSession(secretKey: Parameters<typeof verifyJwtSession>[1]) {
		...
		const requiredFeatures = {
			'sign-transaction': ['rtd:signTransaction', 'rtd:signTransactionBlock'],
			'sign-and-execute-transaction': ['rtd:signAndExecuteTransaction'],
			'sign-personal-message': ['rtd:signPersonalMessage'],
