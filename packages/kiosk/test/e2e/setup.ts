...
export async function setupRtdClient() {
	...
	await retry(() => requestRtdFromFaucetV2({ host: DEFAULT_FAUCET_URL, recipient: address }), {
		backoff: 'EXPONENTIAL',
...
function getCreatedObjectIdByType(res: RtdTransactionBlockResponse, type: string): string {
