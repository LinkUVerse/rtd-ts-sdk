import {
	...
	BcsEnum,
	BcsTuple,
} from 'rtd-typescript/bcs';
import { normalizeStructTag, normalizeRtdAddress } from 'rtd-typescript/utils';
import {
	type TransactionArgument,
	type TransactionObjectArgument,
	isArgument,
} from 'rtd-typescript/transactions';
import { type ClientWithCoreApi, type RtdClientTypes } from 'rtd-typescript/client';

const MOVE_STDLIB_ADDRESS = normalizeRtdAddress('0x1');
const RTD_FRAMEWORK_ADDRESS = normalizeRtdAddress('0x2');

export type RawTransactionArgument<T> = T | TransactionArgument;

export type GetOptions<Include extends Omit<RtdClientTypes.ObjectInclude, 'content'> = {}> =
	RtdClientTypes.GetObjectOptions<Include> & { client: ClientWithCoreApi };

export type GetManyOptions<Include extends Omit<RtdClientTypes.ObjectInclude, 'content'> = {}> =
	RtdClientTypes.GetObjectsOptions<Include> & { client: ClientWithCoreApi };
...
export function getPureBcsSchema(typeTag: string | TypeTag): BcsType<any> | null {
	...
	if ('u8' in parsedTag) {
		...
		const pkg = normalizeRtdAddress(structTag.address);
...
export function normalizeMoveArguments(
	...
	for (const argType of argTypes) {
		...
		if (argType === '0x2::accumulator::AccumulatorRoot') {
			// Chain-wide shared singleton at a fixed address (RTD_ACCUMULATOR_ROOT_OBJECT_ID).
			normalizedArgs.push((tx) => tx.object('0xacc'));
			continue;
		}

		if (argType === '0x3::rtd_system::RtdSystemState') {
...
> extends BcsStruct<T, Name> {
	...
	async get<Include extends Omit<RtdClientTypes.ObjectInclude, 'content' | 'json'> = {}>({
		objectId,
		...options
	}: GetOptions<Include>): Promise<
		RtdClientTypes.Object<Include & { content: true; json: true }> & {
	...
	async getMany<Include extends Omit<RtdClientTypes.ObjectInclude, 'content' | 'json'> = {}>({
		client,
		...options
	...
	> {
		...
		})) as RtdClientTypes.GetObjectsResponse<Include & { content: true }>;
